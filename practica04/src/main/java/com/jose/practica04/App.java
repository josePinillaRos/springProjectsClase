...
public class App {
	...
	private static void borrarCategoria() {
	    ...
	    if (categorias.isEmpty()) {
	        System.out.println("No hay categorías disponibles para borrar.");
	        return;
	    }
	    categorias.forEach(System.out::println);

	    // 2. Solicitar el ID de la categoría a borrar
	    Categories categoriaABorrar = null;
	    while (categoriaABorrar == null) {
	        System.out.print("\nIntroduzca el id de la categoría a borrar: ");
	        String entrada = sc.nextLine();
	        ...
	        try {
	            ...
	            // Buscar la categoría
	            categoriaABorrar = HibernateUtils.getId(Categories.class, id);
	            if (categoriaABorrar == null) {
	                System.out.println("No existe ninguna categoría con ese ID. Inténtelo de nuevo.");
	            }
	        } catch (NumberFormatException e) {
	            System.out.println("Entrada no válida. Por favor, introduzca un número.");
	        }
	    ...
	    // 3. Comprobar los jokes asociados
	    Set<Jokes> jokesAsociados = categoriaABorrar.getJokeses();
	    if (jokesAsociados != null && !jokesAsociados.isEmpty()) {
	        System.out.println("La categoría con ID " + categoriaABorrar.getId() +
	                " está asociada a " + jokesAsociados.size() + " joke(s).");

	        // 3.1 Preguntar qué hacer: desvincular o borrar los jokes
	        System.out.println("¿Qué desea hacer con esos jokes?");
	        System.out.println("1. Desvincular los chistes (poner la categoría a null)");
	        System.out.println("2. Borrar los chistes asociados");
	        System.out.println("3. Cancelar la operación");
	        ...
	        switch (opcion) {
	            case "1":
	                ...
	                for (Jokes j : jokesAsociados) {
	                    j.setCategories(null);
	                    // Actualizar el Joke para no violar la foreign key al eliminar la categoría
	                    HibernateUtils.merge(j);
	                }
	                System.out.println("Se han desvinculado " + jokesAsociados.size() + " chistes de la categoría.");
	                break;

	            case "2":
	                // Borrar los chistes asociados
	                System.out.println("Va a borrar " + jokesAsociados.size() + " chistes asociados a la categoría.");
	                ...
	                // Borrar cada Joke
	                for (Jokes j : jokesAsociados) {
	                    boolean resultadoBorradoJoke = HibernateUtils.remove(j);
	                    if (!resultadoBorradoJoke) {
	                        System.out.println("Error al borrar el chiste con ID: " + j.getId() + ". Operación cancelada.");
	                        return;
	                    }
	                }
	                System.out.println("Se han borrado " + jokesAsociados.size() + " chistes asociados a la categoría.");
	                break;

	            default:
	                // 3.2 Cancelar la operación
	                System.out.println("Operación cancelada. No se ha borrado la categoría.");
	                return;
	        }
	    }
	    ...
	    boolean resultado = HibernateUtils.remove(categoriaABorrar);
	    if (resultado) {
	        System.out.println("Categoría borrada correctamente (ID: " + categoriaABorrar.getId() + ").");
	    } else {
	        System.out.println("Ha ocurrido un error al intentar borrar la categoría.");
	    }
	}
	...
	private static void borrarLenguaje() {
	    ...
	    // 2. Solicitar el ID del lenguaje a borrar
	    Language lenguajeABorrar = null;
	    while (lenguajeABorrar == null) {
	        System.out.print("\nIntroduzca el id del lenguaje a borrar: ");
	        String entrada = sc.nextLine();
	        ...
	        try {
	            ...
	            // Buscar el lenguaje
	            lenguajeABorrar = HibernateUtils.getId(Language.class, id);
	            if (lenguajeABorrar == null) {
	                System.out.println("No existe ningún lenguaje con ese ID. Inténtelo de nuevo.");
	            }
	        } catch (NumberFormatException e) {
	            System.out.println("Entrada no válida. Por favor, introduzca un número.");
	        }
	    ...
	    // 3. Comprobar los jokes asociados
	    Set<Jokes> jokesAsociados = lenguajeABorrar.getJokeses();
	    if (jokesAsociados != null && !jokesAsociados.isEmpty()) {
	        System.out.println("El lenguaje con ID " + lenguajeABorrar.getId() +
	                " está asociado a " + jokesAsociados.size() + " joke(s).");

	        // Preguntar qué hacer: desvincular o borrar los jokes
	        System.out.println("¿Qué desea hacer con esos jokes?");
	        System.out.println("1. Desvincular los chistes (poner el lenguaje a null)");
	        System.out.println("2. Borrar los chistes asociados");
	        System.out.println("3. Cancelar la operación");
	        System.out.print("Opción: ");
	        String opcion = sc.nextLine().trim();
	        ...
	        switch (opcion) {
	            case "1":
	                // Desvincular el lenguaje de cada Joke
	                for (Jokes j : jokesAsociados) {
	                    j.setLanguage(null);
	                    HibernateUtils.merge(j);
	                }
	                System.out.println("Se han desvinculado " + jokesAsociados.size() + " chistes del lenguaje.");
	                break;

	            case "2":
	                // Borrar los jokes asociados
	                System.out.println("Va a borrar " + jokesAsociados.size() + " chistes asociados al lenguaje.");
	                System.out.print("¿Está seguro? (S/N): ");
	                String confirmacion = sc.nextLine().trim().toLowerCase();
	                if (!confirmacion.equals("s")) {
	                    System.out.println("Operación cancelada. No se ha borrado el lenguaje.");
	                    return;
	                }
	                ...
	                System.out.println("Se han borrado " + jokesAsociados.size() + " chistes asociados al lenguaje.");
	                break;
	    ...
	    // 4. Borrar el lenguaje
	    boolean resultado = HibernateUtils.remove(lenguajeABorrar);
	    if (resultado) {
	        System.out.println("Lenguaje borrado correctamente (ID: " + lenguajeABorrar.getId() + ").");
	    } else {
	        System.out.println("Ha ocurrido un error al intentar borrar el lenguaje.");
	    }
	}
	...
	private static void buscarTodosFlags() {
		HibernateUtils.getAll(Flags.class).forEach(System.out::println);
	}
	...
	private static void borrarFlag() {
	    Flags flagABorrar = elegirFlag();
	    if (flagABorrar == null) {
	        return; 
	    }
	
	    List<Jokes> jokesAsociados = getJokesAsociadosAlFlag(flagABorrar);
	
	    // Si hay jokes asociados, dar opciones (desvincular / borrar jokes).
	    //    Si no hay jokes, simplemente se hace el borrado al final.
	    if (!jokesAsociados.isEmpty()) {
	        gestionarJokesAsociados(flagABorrar, jokesAsociados);
	        return; 
	    }
	
	    // Si no había jokes asociados borramos el flag aquí
	    borrarFlagFinal(flagABorrar);
	}
	
	/**
	 * Muestra todos los Flags y solicita al usuario el ID de uno a eliminar.
	 * @return El objeto Flags a eliminar, o null si el usuario no proporciona uno válido.
	 */
	private static Flags elegirFlag() {
	    List<Flags> flags = HibernateUtils.getAll(Flags.class);
	    if (flags.isEmpty()) {
	        System.out.println("No hay flags disponibles para borrar.");
	        return null;
	    }
	    flags.forEach(System.out::println);
	
	    Flags flagABorrar = null;
	    while (flagABorrar == null) {
	        System.out.print("\nIntroduzca el id del flag a borrar: ");
	        String entrada = sc.nextLine();
	
	        try {
	            int id = Integer.parseInt(entrada);
	            flagABorrar = HibernateUtils.getId(Flags.class, id);
	            if (flagABorrar == null) {
	                System.out.println("No existe ningún flag con ese ID. Inténtelo de nuevo.");
	            }
	        } catch (NumberFormatException e) {
	            System.out.println("Entrada no válida. Por favor, introduzca un número.");
	        }
	 ...
	 */
	private static List<Jokes> getJokesAsociadosAlFlag(Flags flagABorrar) {
	    HashMap<String, Object> params = new HashMap<>();
	    params.put("flagId", flagABorrar.getId());
	
	    return HibernateUtils.nativeQuery(
	        "SELECT j.* " +
	        "FROM jokes j " +
	        "JOIN jokes_flags jf ON j.id = jf.joke_id " +
	        "WHERE jf.flag_id = :flagId",
	        Jokes.class,
	        params
	    );
	}
	 ...
	 */
	private static void borrarFlagFinal(Flags flagABorrar) {
		 boolean confirmado = leerConfirmacionSN(
		            "Va a borrar el flag con ID: " + flagABorrar.getId() + ". ¿Está seguro? (S/N): "
		    );
		    if (!confirmado) {
		        System.out.println("Operación cancelada. No se ha borrado el flag.");
		        return;
		    ...
		    boolean resultado = HibernateUtils.remove(flagABorrar);
		    if (resultado) {
		        System.out.println("Flag borrado correctamente (ID: " + flagABorrar.getId() + ").");
		    } else {
		        System.out.println("Ha ocurrido un error al intentar borrar el flag.");
		    }
	}
